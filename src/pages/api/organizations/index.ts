...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.advertisement_script?.length > 0) {
      const create_advertisement_script = body.advertisement_script;
      body.advertisement_script = {
        create: create_advertisement_script,
      };
    } else {
      delete body.advertisement_script;
    }
